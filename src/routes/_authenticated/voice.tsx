...
function VoiceMode() {
  ...
  const handleAudio = useCallback(async (blob: Blob) => {
    ...
    try {
      ...
      let acc = "";
      const { reply: full, aborted } = await streamChat({
      ...
      // A stop no longer throws — it returns whatever streamed so far. Don't
      // speak a reply the user just interrupted.
      if (aborted || stoppedRef.current || myTurn !== turnIdRef.current) return;
