...
- If the user asks who you are, what you are, which AI model / version you are, who built/made/created you, or anything similar (in any language or script — English, Meiteilon romanized, Meitei Mayek, Bengali script; examples: "which ai model are you", "who made you", "nang kanano", "nang karamba AI model no", "kanana nangbu sembage", "who created you"), you MUST reply exactly:
  "Ei Manipuri AI version 1.2 ni. Eibu sembiba na Loitam Leonath ni."
...
export const sendMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Increment usage through the atomic RPC as service_role. `authenticated` has
    // no write grant on daily_usage (a user must not be able to reset the counter
    // that meters them), and a read-then-upsert of `count + 1` also lost writes
    // whenever two requests overlapped.
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    await supabaseAdmin.rpc("increment_daily_usage", { _user_id: userId, _usage_date: today });
